		      ...
		      char **error_message ATTRIBUTE_UNUSED)
{
  ...
  if (address > bfd_get_section_limit (abfd, input_section))
  ...
  data_ptr = (bfd_byte *) data + address;
...
elf_s390_link_hash_table_create (bfd *abfd)
{
  ...
  ret = bfd_zmalloc (sizeof (struct elf_s390_link_hash_table));
  if (ret == NULL)
    return NULL;
			 ...
			 int is_local)
{
  ...
  if (r_type == R_390_TLS_GD32 || r_type == R_390_TLS_IE32)
    return is_local ? R_390_TLS_LE32 : R_390_TLS_IE32;

  if (r_type == R_390_TLS_GOTIE32)
  ...
  for (rel = relocs; rel < rel_end; rel++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (ELF_ST_TYPE (isym->st_info) == STT_GNU_IFUNC)
	    {
	      if (!elf_s390_handle_local_ifunc (abfd, htab, info, 
						symtab_hdr, r_symndx))
		return false;
	      local_got_refcounts = elf_local_got_refcounts (abfd);
	    }
	...
	{
	  h = elf_s390_get_indirect_symbol (sym_hashes, r_symndx, symtab_hdr);
	}
      ...
      if (!elf_s390_create_got_section_if_needed (abfd, htab, info, 
						   r_type, h, 
						   &local_got_refcounts, 
						   symtab_hdr))
	return false;
      ...
      if (h != NULL)
	{
	  if (!elf_s390_handle_ifunc_symbol (htab, info, h))
	    return false;
	}

      if (!elf_s390_process_reloc_type (abfd, info, sec, rel, r_type, 
					r_symndx, h, htab, 
					local_got_refcounts, 
					symtab_hdr, &sreloc))
	return false;
...
elf_s390_get_indirect_symbol (struct elf_link_hash_entry **sym_hashes,
			      unsigned int r_symndx,
			      Elf_Internal_Shdr *symtab_hdr)
{
...
static bool
elf_s390_handle_local_ifunc (bfd *abfd,
			     struct elf_s390_link_hash_table *htab,
			     struct bfd_link_info *info,
			     Elf_Internal_Shdr *symtab_hdr,
			     unsigned int r_symndx)
...
static bool
elf_s390_handle_ifunc_symbol (struct elf_s390_link_hash_table *htab,
			      struct bfd_link_info *info,
			      struct elf_link_hash_entry *h)
{
  if (htab->elf.dynobj == NULL)
    htab->elf.dynobj = h->root.u.def.section->owner;
...
static bool
elf_s390_needs_got_section (unsigned int r_type)
{
  switch (r_type)
    {
    case R_390_GOT12:
...
static bool
elf_s390_needs_local_got_refcounts (unsigned int r_type)
{
  switch (r_type)
    {
    case R_390_GOT12:
...
static bool
elf_s390_create_got_section_if_needed (bfd *abfd,
				       struct elf_s390_link_hash_table *htab,
				       struct bfd_link_info *info,
				       unsigned int r_type,
				       struct elf_link_hash_entry *h,
				       ...
				       Elf_Internal_Shdr *symtab_hdr)
{
  if (elf_s390_needs_local_got_refcounts (r_type))
    {
  ...
  if (elf_s390_needs_got_section (r_type))
    {
...
static bool
elf_s390_process_reloc_type (bfd *abfd,
			     struct bfd_link_info *info,
			     asection *sec,
			     const Elf_Internal_Rela *rel,
			     unsigned int r_type,
			     ...
			     asection **sreloc)
{
  switch (r_type)
    ...
    case R_390_PLTOFF16:
    case R_390_PLTOFF32:
      if (!elf_s390_process_plt_reloc (h))
	return false;
      break;
    ...
    case R_390_GOTPLT32:
    case R_390_GOTPLTENT:
      if (!elf_s390_process_gotplt_reloc (h, local_got_refcounts, r_symndx))
	return false;
      break;
    ...
    case R_390_GOTENT:
    case R_390_TLS_GD32:
      if (!elf_s390_process_got_reloc (abfd, r_type, h, 
				       local_got_refcounts, r_symndx))
	return false;
      if (r_type != R_390_TLS_IE32)
	break;
    case R_390_TLS_LE32:
      if (!elf_s390_process_tls_le_reloc (r_type, info))
	return false;
    case R_390_8:
    case R_390_16:
    case R_390_32:
    ...
    case R_390_PC32DBL:
    case R_390_PC32:
      if (!elf_s390_process_dyn_reloc (abfd, info, sec, rel, r_type, 
				       r_symndx, h, htab, symtab_hdr, sreloc))
	return false;
      break;
...
static bool
elf_s390_process_plt_reloc (struct elf_link_hash_entry *h)
{
  if (h != NULL)
    {
      h->needs_plt = 1;
...
static bool
elf_s390_process_gotplt_reloc (struct elf_link_hash_entry *h,
			       bfd_signed_vma *local_got_refcounts,
			       unsigned int r_symndx)
{
  if (h != NULL)
...
static int
elf_s390_get_tls_type (unsigned int r_type)
{
  switch (r_type)
    {
    case R_390_TLS_GD32:
...
static bool
elf_s390_process_got_reloc (bfd *abfd,
			    unsigned int r_type,
			    struct elf_link_hash_entry *h,
			    bfd_signed_vma *local_got_refcounts,
			    unsigned int r_symndx)
{
  ...
  tls_type = elf_s390_get_tls_type (r_type);
...
static bool
elf_s390_process_tls_le_reloc (unsigned int r_type,
			       struct bfd_link_info *info)
{
...
static bool
elf_s390_is_pc_relative (unsigned int r_type)
{
...
static bool
elf_s390_needs_copy_reloc (struct bfd_link_info *info,
			   struct elf_link_hash_entry *h,
			   asection *sec,
			   unsigned int r_type)
{
  ...
  if (bfd_link_pic (info))
    {
      if (!elf_s390_is_pc_relative (r_type))
	return true;
      if (h != NULL
...
static bool
elf_s390_process_dyn_reloc (bfd *abfd,
			    struct bfd_link_info *info,
			    asection *sec,
			    const Elf_Internal_Rela *rel,
			    unsigned int r_type,
			    ...
			    asection **sreloc)
{
  ...
  if (elf_s390_needs_copy_reloc (info, h, sec, r_type))
    {
      if (!elf_s390_create_dyn_reloc (abfd, info, sec, rel, r_type, 
				      r_symndx, h, htab, symtab_hdr, sreloc))
	return false;
    }
  return true;
...
static bool
elf_s390_create_dyn_reloc (bfd *abfd,
			   struct bfd_link_info *info,
			   asection *sec,
			   const Elf_Internal_Rela *rel,
			   unsigned int r_type,
			     ...
			     struct bfd_link_info *info)
{
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next)
	...
	continue;
      ...
      if (!process_local_sections (ibfd, htab, info))
	return false;
    }
  ...
  for (s = dynobj->sections; s != NULL; s = s->next)
    {
      if (!process_dynamic_section (s, htab, dynobj, &relocs))
	return false;
    }
...
static bool
process_local_sections (bfd *ibfd, struct elf_s390_link_hash_table *htab,
			struct bfd_link_info *info)
{
...
static bool
process_dynamic_section (asection *s, struct elf_s390_link_hash_table *htab,
			bfd *dynobj, bool *relocs)
{
  ...
  for (; rel < relend; rel++)
    {
      if (!process_single_relocation(output_bfd, info, input_bfd, input_section,
                                     ...
                                     htab, symtab_hdr, sym_hashes, local_got_offsets))
        return false;
    }
                          ...
                          bfd_vma *local_got_offsets)
{
  ...
  if (r_type == (int) R_390_GNU_VTINHERIT || r_type == (int) R_390_GNU_VTENTRY)
    return true;
  ...
  if (r_symndx < symtab_hdr->sh_info)
    {
      if (!process_local_symbol(output_bfd, input_bfd, rel, r_symndx, r_type,
                                local_syms, local_sections, local_got_offsets,
                                htab, &sym, &sec, &relocation))
        return false;
    }
  else
    {
  ...
  if (!apply_relocation(output_bfd, info, input_bfd, input_section,
                        contents, rel, howto, r_type, r_symndx,
                        h, sym, sec, relocation, unresolved_reloc,
                        resolved_to_zero, htab, symtab_hdr,
                        local_got_offsets, base_got))
    return false;
...
static bool
process_local_symbol(bfd *output_bfd, bfd *input_bfd, Elf_Internal_Rela *rel,
                    unsigned long r_symndx, unsigned int r_type,
                    Elf_Internal_Sym *local_syms, asection **local_sections,
                    bfd_vma *local_got_offsets, struct elf_s390_link_hash_table *htab,
                    Elf_Internal_Sym **sym_out, asection **sec_out, bfd_vma *relocation_out)
{
  *sym_out = local_syms + r_symndx;
  *sec_out = local_sections[r_symndx];

  if (ELF_ST_TYPE ((*sym_out)->st_info) == STT_GNU_IFUNC)
    {
      return handle_local_ifunc(output_bfd, input_bfd, rel, r_symndx, r_type,
                               local_got_offsets, htab, *sec_out, relocation_out);
    }

  *relocation_out = _bfd_elf_rela_local_sym (output_bfd, *sym_out, sec_out, rel);
  return true;
}

static bool
handle_local_ifunc(bfd *output_bfd, bfd *input_bfd, Elf_Internal_Rela *rel,
                  unsigned long r_symndx, unsigned int r_type,
                  bfd_vma *local_got_offsets, struct elf_s390_link_hash_table *htab,
                  asection *sec, bfd_vma *relocation_out)
{
  struct plt_entry *local_plt = elf_s390_local_plt (input_bfd);
  if (local_plt == NULL)
    return false;

  *relocation_out = (htab->elf.iplt->output_section->vma +
  ...
  switch (r_type)
    ...
    case R_390_PLTOFF16:
    case R_390_PLTOFF32:
      *relocation_out -= htab->elf.sgot->output_section->vma;
      break;
    ...
    case R_390_GOT32:
    case R_390_GOTENT:
      bfd_put_32 (output_bfd, *relocation_out,
                 htab->elf.sgot->contents + local_got_offsets[r_symndx]);
      *relocation_out = (local_got_offsets[r_symndx] +
...
apply_relocation(bfd *output_bfd, struct bfd_link_info *info,
                bfd *input_bfd, asection *input_section,
                bfd_byte *contents, Elf_Internal_Rela *rel,
                reloc_howto_type *howto, unsigned int r_type,
                unsigned long r_symndx, struct elf_link_hash_entry *h,
                Elf_Internal_Sym *sym, asection *sec,
                bfd_vma relocation, bool unresolved_reloc,
                bool resolved_to_zero, struct elf_s390_link_hash_table *htab,
                Elf_Internal_Shdr *symtab_hdr, bfd_vma *local_got_offsets,
                asection *base_got)
{
  switch (r_type)
    {
    ...
    case R_390_GOTPLT32:
    case R_390_GOTPLTENT:
      if (!handle_gotplt_reloc(output_bfd, info, h, htab, r_type,
    ...
    case R_390_GOT32:
    case R_390_GOTENT:
      if (!handle_got_reloc(output_bfd, info, input_bfd, input_section,
                           contents, rel, howto, r_type, r_symndx,
                           h, htab, local_got_offsets, base_got,
                           &relocation, &unresolved_reloc, resolved_to_zero))
        return false;
      break;

    case R_390_GOTOFF16:
    case R_390_GOTOFF32:
      if (!handle_gotoff_reloc(info, h, htab, &relocation))
        return false;
      break;
    ...
    case R_390_PLT32DBL:
    case R_390_PLT32:
      if (!handle_plt_reloc(h, htab, &relocation, &unresolved_reloc))
        return false;
      break;

    case R_390_PLTOFF16:
    case R_390_PLTOFF32:
      if (!handle_pltoff_reloc(h, htab, &relocation, &unresolved_reloc))
        return false;
      break;
    ...
    case R_390_PC32DBL:
    case R_390_PC32:
      if (!handle_pc_reloc(info, h, htab, &relocation))
        return false;

    case R_390_8:
    case R_390_16:
    case R_390_32:
      if (!handle_data_reloc(output_bfd, info, input_bfd, input_section,
                            contents, rel, r_type, r_symndx, h, sym, sec,
                            &relocation, &unresolved_reloc, resolved_to_zero,
                            htab, symtab_hdr))
        return false;
      break;
    ...
    case R_390_TLS_LDCALL:
      if (!handle_tls_reloc(output_bfd, info, input_bfd, input_section,
                           contents, rel, r_type, r_symndx, h,
                           htab, local_got_offsets, &relocation,
                           &unresolved_reloc))
        return false;
      break;
  ...
  if (unresolved_reloc && !((input_section->flags & SEC_DEBUGGING) != 0 && h->def_dynamic) &&
  ...
  return perform_final_relocation(output_bfd, info, input_bfd, input_section,
                                 contents, rel, howto, r_type, r_symndx,
                                 h, sym, sec, relocation, symtab_hdr);
}
...
handle_gotplt_reloc(bfd *output_bfd, struct bfd_link_info *info,
                   struct elf_link_hash_entry *h,
                   struct elf_s390_link_hash_table *htab,
                   unsigned int r_type, bfd_vma *relocation,
                   bool *unresolved_reloc)
{
  if (h == NULL || h->plt.offset == (bfd_vma) -1)
    return true;
...
handle_got_reloc(bfd *output_bfd, struct bfd_link_info *info,
                ...
                unsigned long r_symndx, struct elf_link_hash_entry *h,
                struct elf_s390_link_hash_table *htab,
                bfd_vma *local_got_offsets, asection *base_got,
                bfd_vma *relocation, bool *unresolved_reloc,
                bool resolved_to_zero)
{
  bfd_vma off;
  ...
  if (h != NULL)
    {
      if (!handle_global_got_entry(output_bfd, info, input_section, contents,
                                   rel, howto, r_type, h, htab, base_got,
                                   relocation, unresolved_reloc, resolved_to_zero, &off))
        return false;
    }
  else
    {
      if (!handle_local_got_entry(output_bfd, info, r_symndx, htab,
...
static bool
handle_global_got_entry(bfd *output_bfd, struct bfd_link_info *info,
                       asection *input_section, bfd_byte *contents,
                       Elf_Internal_Rela *rel, reloc_howto_type *howto,
                       unsigned int r_type, struct elf_link_hash_entry *h,
                       struct elf_s390_link_hash_table *htab,
                       asection **base_got, bfd_vma *relocation,
                       bool *unresolved_reloc, bool resolved_to_zero,
                       bfd_vma *off_out)
{
  bool dyn;
  ...
  if (s390_is_ifunc_symbol_p (h))
           ...
           SYMBOL_REFERENCES_LOCAL (info, h) || resolved_to_zero)
    {
      ...
      if (h->def_regular && SYMBOL_REFERENCES_LOCAL (info, h) &&
          try_got_to_larl_transformation(input_bfd, contents, rel, r_type, h,
                                        relocation, &r_type, howto))
        {
          *unresolved_reloc = false;
          *off_out = off;
          return true;
...
static bool
try_got_to_larl_transformation(bfd *input_bfd, bfd_byte *contents,
                              Elf_Internal_Rela *rel, unsigned int r_type,
                              struct elf_link_hash_entry *h,
                              bfd_vma *relocation, unsigned int *new_r_type,
                              reloc_howto_type **howto)
{
  if (r_type == R_390_GOTENT &&
      (bfd_get_16 (input_bfd, contents + rel->r_offset - 2) & 0xff0f) == 0xc40d)
    {
      unsigned short new_insn = (0xc000 |
                                 (bfd_get_8 (input_bfd, contents + rel->r_offset - 1) & 0xf0));
      bfd_put_16 (input_bfd, new_insn, contents + rel->r_offset - 2);
      *new_r_type = R_390_PC32DBL;
      rel->r_addend = 2;
      *howto = elf_howto_table + *new_r_type;
...
static bool
handle_local_got_entry(bfd *output_bfd, struct bfd_link_info *info,
                      unsigned long r_symndx, struct elf_s390_link_hash_table *htab,
                      ...
                      bfd_vma *off_out)
{
  ...
  if ((off & 1) != 0)
  ...
  else
    {
      ...
      if (bfd_link_pic (info))
        {
          if (!emit_got_relocation(output_bfd, htab, off, relocation))
            return false;
        }
...
static bool
emit_got_relocation(bfd *output_bfd, struct elf_s390_link_hash_table *htab,
...
static bool
handle_gotoff_reloc(struct bfd_link_info *info, struct elf_link_hash_entry *h,
                   struct elf_s390_link_hash_table *htab, bfd_vma *relocation)
{
  if (h != NULL && s390_is_ifunc_symbol_p (h) && h->def_regular &&
...
static bool
handle_pltoff_reloc(struct elf_link_hash_entry *h,
                   struct elf_s390_link_hash_table *htab,
                   bfd_vma *relocation, bool *unresolved_reloc)
{
  if (h == NULL || h->plt.offset == (bfd_vma) -1 ||
...
static bool
handle_pc_reloc(struct bfd_link_info *info, struct elf_link_hash_entry *h,
               struct elf_s390_link_hash_table *htab, bfd_vma *relocation)
{
  if (h != NULL && s390_is_ifunc_symbol_p (h) && h->def_regular &&
      !bfd_link_executable (info))
...
static bool
handle_data_reloc(bfd *output_bfd, struct bfd_link_info *info,
                 bfd *input_bfd, asection *input_section,
                 bfd_byte *contents, Elf_Internal_Rela *rel,
                 ...
                 Elf_Internal_Shdr *symtab_hdr)
{
  ...
  if (h != NULL && s390_is_ifunc_symbol_p (h) && h->def_regular)
    {
      return handle_ifunc_data_reloc(output_bfd, info, input_bfd,
                                    input_section, rel, r_type, h, htab);
    }

  if (needs_dynamic_reloc(info, input_section, rel, r_type, h, resolved_to_zero))
    {
      return emit_dynamic_reloc(output_bfd, info, input_bfd, input_section,
                              rel, r_type, h, sec, relocation, htab);
    }
...
static bool
handle_ifunc_data_reloc(bfd *output_bfd, struct bfd_link_info *info,
                       bfd *input_bfd, asection *input_section,
                       Elf_Internal_Rela *rel, unsigned int r_type,
                       struct elf_link_hash_entry *h,
                       struct elf_s390_link_hash_table *htab)
			      ...
			      bfd_vma resolver_address)
{
  ...
  if (!bfd_link_pic (info))
  ...
  else if (got_offset < 4096)
    {
      ...
      bfd_put_16 (output_bfd, 0xc000 | got_offset,
				...
				Elf_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
	...
	{
	  if (!elf_s390_setup_plt_entry(output_bfd, info, h, htab, sym))
	    return false;
	}
    }
  ...
  if (h->got.offset != (bfd_vma) -1
    ...
    {
      if (!elf_s390_setup_got_entry(output_bfd, info, h, htab))
	return false;
    }

  if (h->needs_copy)
    {
      if (!elf_s390_setup_copy_reloc(output_bfd, h, htab))
	return false;
    }
...
static bool
elf_s390_setup_plt_entry(bfd *output_bfd,
			  struct bfd_link_info *info,
			  struct elf_link_hash_entry *h,
			  struct elf_s390_link_hash_table *htab,
			  Elf_Internal_Sym *sym)
{
  ...
  if (!elf_s390_fill_plt_entry(output_bfd, info, htab, h, got_offset, relative_offset))
    return false;
...
static bool
elf_s390_fill_plt_entry(bfd *output_bfd,
			 struct bfd_link_info *info,
			 struct elf_s390_link_hash_table *htab,
			 struct elf_link_hash_entry *h,
			 bfd_vma got_offset,
...
static bool
elf_s390_setup_got_entry(bfd *output_bfd,
			  struct bfd_link_info *info,
			  struct elf_link_hash_entry *h,
			  struct elf_s390_link_hash_table *htab)
{
...
static bool
elf_s390_setup_copy_reloc(bfd *output_bfd,
			   struct elf_link_hash_entry *h,
			   struct elf_s390_link_hash_table *htab)
{
  Elf_Internal_Rela rela;
				  ...
				  struct bfd_link_info *info)
{
  ...
  if (htab->elf.dynamic_sections_created)
    {
      if (!elf_s390_process_dynamic_entries(output_bfd, htab, dynobj, sdyn))
        return false;

      if (!elf_s390_fill_plt_first_entry(output_bfd, htab, info))
        return false;
    }

  if (!elf_s390_fill_gotplt_entries(output_bfd, htab, sdyn))
    return false;

  if (!elf_s390_finish_local_ifunc_symbols(output_bfd, info, htab))
    return false;
...
static bool
elf_s390_process_dynamic_entries(bfd *output_bfd,
                                 struct elf_s390_link_hash_table *htab,
                                 bfd *dynobj,
                                 asection *sdyn)
{
...
static bool
elf_s390_fill_plt_first_entry(bfd *output_bfd,
                              struct elf_s390_link_hash_table *htab,
                              struct bfd_link_info *info)
{
...
static bool
elf_s390_fill_gotplt_entries(bfd *output_bfd,
                             struct elf_s390_link_hash_table *htab,
                             asection *sdyn)
{
  if (htab->elf.sgotplt == NULL || htab->elf.sgotplt->size == 0)
    return true;

  bfd_vma dynamic_addr = (sdyn == NULL) ? (bfd_vma) 0
...
static bool
elf_s390_finish_local_ifunc_symbols(bfd *output_bfd,
                                    struct bfd_link_info *info,
                                    struct elf_s390_link_hash_table *htab)
{
  bfd *ibfd;
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next)
    {
      ...
      if (!elf_s390_process_local_plt_entries(output_bfd, info, htab, ibfd))
        return false;
    }
...
static bool
elf_s390_process_local_plt_entries(bfd *output_bfd,
                                   struct bfd_link_info *info,
                                   struct elf_s390_link_hash_table *htab,
                                   bfd *ibfd)
{
...
{
  const int S390_LINUX_NOTE_SIZE = 224;
  const int SIGNAL_OFFSET = 12;
  const int PID_OFFSET = 24;
  const int REGISTER_OFFSET = 72;
  const int REGISTER_SIZE = 144;

  if (note->descsz != S390_LINUX_NOTE_SIZE)
    return false;
