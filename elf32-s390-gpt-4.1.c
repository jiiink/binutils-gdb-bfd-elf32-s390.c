...
static reloc_howto_type *elf_s390_reloc_type_lookup(bfd *abfd ATTRIBUTE_UNUSED, bfd_reloc_code_real_type code)
{
    static const struct {
        ...
        {BFD_RELOC_VTABLE_INHERIT,   -1,  &elf32_s390_vtinherit_howto},
        {BFD_RELOC_VTABLE_ENTRY,     -1,  &elf32_s390_vtentry_howto}
    };
    size_t i;
    for (i = 0; i < sizeof(howto_map)/sizeof(howto_map[0]); ++i) {
        if (howto_map[i].code == code) {
            if (howto_map[i].howto_ptr)
                return howto_map[i].howto_ptr;
            if (howto_map[i].howto_index >= 0)
                return &elf_howto_table[howto_map[i].howto_index];
            return 0;
        }
    }
    return 0;
...
elf_s390_reloc_name_lookup(bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
    ...
    for (i = 0; i < table_size; i++) {
        const char *entry_name = elf_howto_table[i].name;
        if (entry_name && strcasecmp(entry_name, r_name) == 0)
            return &elf_howto_table[i];
    }

    if (elf32_s390_vtinherit_howto.name &&
        strcasecmp(elf32_s390_vtinherit_howto.name, r_name) == 0)
        return &elf32_s390_vtinherit_howto;

    if (elf32_s390_vtentry_howto.name &&
...
static bool elf_s390_info_to_howto(bfd *abfd, arelent *cache_ptr, Elf_Internal_Rela *dst) {
...
static bool elf_s390_mkobject(bfd *abfd) {
  ...
  return bfd_elf_allocate_object(abfd, sizeof(struct elf_s390_obj_tdata)) != 0;
}
...
static bool elf_s390_object_p(bfd *abfd)
{
    if (!abfd)
        return false;
    return bfd_default_set_arch_mach(abfd, bfd_arch_s390, bfd_mach_s390_31) ? true : false;
}
...
static void elf_s390_copy_indirect_symbol(struct bfd_link_info *info,
                      ...
                      const Elf_Internal_Rela *relocs)
{
    ...
    for (rel = relocs; rel < rel_end; rel++) {
        ...
        switch (r_type) {
            ...
            case R_390_PC32: {
                bool need_copy;
                need_copy = ((bfd_link_pic(info) && (sec->flags & SEC_ALLOC) &&
                             ...
                             (h != NULL &&
                              (!SYMBOLIC_BIND(info, h) ||
                               ...
                               !h->def_regular)))
                            || (ELIMINATE_COPY_RELOCS && !bfd_link_pic(info) &&
                                (sec->flags & SEC_ALLOC) && h != NULL &&
                                (h->root.type == bfd_link_hash_defweak ||
                                 !h->def_regular));
...
static void elf_s390_adjust_gotplt(struct elf_s390_link_hash_entry *h)
{
    ...
    if (h->elf.root.type == bfd_link_hash_warning && h->elf.root.u.i.link != NULL)
...
static bool
elf_s390_adjust_dynamic_symbol(struct bfd_link_info *info, struct elf_link_hash_entry *h)
{
    struct elf_s390_link_hash_table *htab;
...
static bool elf_s390_late_size_sections(bfd *output_bfd ATTRIBUTE_UNUSED, struct bfd_link_info *info)
{
  struct elf_s390_link_hash_table *htab = elf_s390_hash_table(info);
  ...
  for (asection *s = dynobj->sections; s != NULL; s = s->next) {
    ...
    if (s == htab->elf.splt || s == htab->elf.sgot || s == htab->elf.sgotplt ||
        s == htab->elf.sdynbss || s == htab->elf.sdynrelro || s == htab->elf.iplt ||
        s == htab->elf.igotplt || s == htab->irelifunc) {
...
static int elf_s390_relocate_section(bfd *output_bfd,
                                     struct bfd_link_info *info,
                                     bfd *input_bfd,
                                     asection *input_section,
                                     bfd_byte *contents,
  ...
  for (; rel < relend; rel++) {
    ...
    switch (r_type) {
      ...
      case R_390_GOT32: case R_390_GOTENT: {
        if (!base_got) abort();
        if (h) {
          int dyn = htab->elf.dynamic_sections_created;
          off = h->got.offset;
          if (s390_is_ifunc_symbol_p(h)) {
      ...
      case R_390_GOTOFF16: case R_390_GOTOFF32: {
        if (h && s390_is_ifunc_symbol_p(h) && h->def_regular && !bfd_link_executable(info)) {
          relocation = htab->elf.iplt->output_section->vma +
                       htab->elf.iplt->output_offset +
      ...
      case R_390_8: case R_390_16: case R_390_32: {
        if (!(input_section->flags & SEC_ALLOC))
          break;
        if (h && s390_is_ifunc_symbol_p(h) && h->def_regular) {
          if (!bfd_link_pic(info)) {
            relocation = htab->elf.iplt->output_section->vma +
                         htab->elf.iplt->output_offset +
                         h->plt.offset;
        ...
        if ((bfd_link_pic(info) &&
             (!h || (ELF_ST_VISIBILITY(h->other) == STV_DEFAULT && !resolved_to_zero) || h->root.type != bfd_link_hash_undefweak) &&
             ((r_type != R_390_PC16 && r_type != R_390_PC12DBL && r_type != R_390_PC16DBL && r_type != R_390_PC24DBL &&
               r_type != R_390_PC32DBL && r_type != R_390_PC32) ||
              !SYMBOL_CALLS_LOCAL(info, h)))
            ||
            (ELIMINATE_COPY_RELOCS && !bfd_link_pic(info) && h && h->dynindx != -1 && !h->non_got_ref &&
             ((h->def_dynamic && !h->def_regular) || h->root.type == bfd_link_hash_undefweak ||
              h->root.type == bfd_link_hash_undefined))) {
          Elf_Internal_Rela outrel;
          ...
          else if (h && h->dynindx != -1 &&
                   (r_type == R_390_PC16 || r_type == R_390_PC12DBL ||
      ...
      case R_390_TLS_IE32: {
        if (bfd_link_pic(info)) {
          ...
          asection *sreloc;
          bfd_byte *loc;
          outrel.r_offset = rel->r_offset + input_section->output_section->vma + input_section->output_offset;
          outrel.r_info = ELF32_R_INFO(0, R_390_RELATIVE);
          sreloc = elf_section_data(input_section)->sreloc;
          if (!sreloc) abort();
          loc = sreloc->contents + sreloc->reloc_count++ * sizeof(Elf32_External_Rela);
      ...
      case R_390_TLS_GD32: case R_390_TLS_GOTIE32: {
        r_type = elf_s390_tls_transition(info, r_type, !h);
        tls_type = GOT_UNKNOWN;
        if (!h && local_got_offsets)
        ...
        off = h ? h->got.offset : (local_got_offsets ? local_got_offsets[r_symndx] : 0);
emit_tls_relocs:
        if (off & 1)
          off &= ~1;
        else {
          ...
          dr_type = (r_type == R_390_TLS_GD32) ? R_390_TLS_DTPMOD : R_390_TLS_TPOFF;
      ...
      case R_390_TLS_LDCALL: {
        ...
        if (r_type == R_390_TLS_LOAD) {
          if (!bfd_link_pic(info) && (!h || h->dynindx == -1)) {
...
do_relocation:
    ...
    if (r_type == R_390_20 || r_type == R_390_GOT20 ||
...
static void elf_s390_finish_ifunc_symbol(bfd *output_bfd,
                                         struct bfd_link_info *info,
                                         struct elf_link_hash_entry *h,
                                         struct elf_s390_link_hash_table *htab,
                                         bfd_vma iplt_offset,
...
{
    if (!output_bfd || !info || !htab || !htab->elf.iplt || !htab->elf.igotplt || !htab->elf.irelplt) {
        abort();
    }
    ...
    bfd_vma plt_entry_addr = (plt->output_offset + (PLT_ENTRY_SIZE * iplt_index) + 18);
    bfd_vma relative_offset = -(plt_entry_addr / 2);

    if ((int)relative_offset < -32768) {
        relative_offset = -(unsigned)(((65536 / PLT_ENTRY_SIZE - 1) * PLT_ENTRY_SIZE) / 2);
    }

    bfd_byte *plt_loc = plt->contents + iplt_offset;

    if (!bfd_link_pic(info)) {
        memcpy(plt_loc, elf_s390_plt_entry, PLT_ENTRY_SIZE);
        bfd_put_32(output_bfd, (bfd_vma)(relative_offset << 16), plt_loc + 20);
        bfd_put_32(output_bfd, gotplt->output_section->vma + got_offset, plt_loc + 24);
    } else if (got_offset < 4096) {
        memcpy(plt_loc, elf_s390_plt_pic12_entry, PLT_ENTRY_SIZE);
        bfd_put_16(output_bfd, (bfd_vma)0xc000 | got_offset, plt_loc + 2);
        bfd_put_32(output_bfd, (bfd_vma)(relative_offset << 16), plt_loc + 20);
    } else if (got_offset < 32768) {
        memcpy(plt_loc, elf_s390_plt_pic16_entry, PLT_ENTRY_SIZE);
        bfd_put_16(output_bfd, (bfd_vma)got_offset, plt_loc + 2);
        bfd_put_32(output_bfd, (bfd_vma)(relative_offset << 16), plt_loc + 20);
    } else {
        memcpy(plt_loc, elf_s390_plt_pic_entry, PLT_ENTRY_SIZE);
        bfd_put_32(output_bfd, (bfd_vma)(relative_offset << 16), plt_loc + 20);
        bfd_put_32(output_bfd, got_offset, plt_loc + 24);
    }

    bfd_put_32(output_bfd, relplt->output_offset + iplt_index * RELA_ENTRY_SIZE, plt_loc + 28);
...
elf_s390_finish_dynamic_symbol(bfd *output_bfd,
    ...
    if (h->plt.offset != (bfd_vma)-1) {
        ...
        if (s390_is_ifunc_symbol_p(h) && h->def_regular) {
            ...
            if (!bfd_link_pic(info)) {
                ...
                bfd_put_32(output_bfd, (bfd_vma)(relative_offset << 16), htab->elf.splt->contents + h->plt.offset + 20);
                bfd_put_32(output_bfd, htab->elf.sgotplt->output_section->vma + htab->elf.sgotplt->output_offset + got_offset,
                           htab->elf.splt->contents + h->plt.offset + 24);
            } else if (got_offset < 4096) {
                memcpy(htab->elf.splt->contents + h->plt.offset, elf_s390_plt_pic12_entry, PLT_ENTRY_SIZE);
                bfd_put_16(output_bfd, (bfd_vma)0xc000 | got_offset, htab->elf.splt->contents + h->plt.offset + 2);
                bfd_put_32(output_bfd, (bfd_vma)(relative_offset << 16), htab->elf.splt->contents + h->plt.offset + 20);
            } else if (got_offset < 32768) {
                memcpy(htab->elf.splt->contents + h->plt.offset, elf_s390_plt_pic16_entry, PLT_ENTRY_SIZE);
                bfd_put_16(output_bfd, (bfd_vma)got_offset, htab->elf.splt->contents + h->plt.offset + 2);
                bfd_put_32(output_bfd, (bfd_vma)(relative_offset << 16), htab->elf.splt->contents + h->plt.offset + 20);
            } else {
                memcpy(htab->elf.splt->contents + h->plt.offset, elf_s390_plt_pic_entry, PLT_ENTRY_SIZE);
                bfd_put_32(output_bfd, (bfd_vma)(relative_offset << 16), htab->elf.splt->contents + h->plt.offset + 20);
                bfd_put_32(output_bfd, got_offset, htab->elf.splt->contents + h->plt.offset + 24);
            }
    ...
    if (h->got.offset != (bfd_vma)-1 &&
        ...
        rela.r_offset = htab->elf.sgot->output_section->vma +
                        htab->elf.sgot->output_offset + (h->got.offset & ~(bfd_vma)1);
                         ...
                         const Elf_Internal_Rela *rela)
{
    ...
    {
        case R_390_RELATIVE: return reloc_class_relative;
        case R_390_JMP_SLOT: return reloc_class_plt;
        case R_390_COPY:     return reloc_class_copy;
...
static bool elf_s390_finish_dynamic_sections(bfd *output_bfd, struct bfd_link_info *info)
{
        ...
        for (; dyncon < dynconend; dyncon++) {
            ...
            switch (dyn.d_tag) {
                ...
                case DT_JMPREL:
                    ...
                    break;
                case DT_PLTRELSZ:
                    dyn.d_un.d_val = (htab->elf.srelplt ? htab->elf.srelplt->size : 0);
                    if (htab->elf.irelplt)
    ...
    if (htab->elf.sgotplt) {
        if (htab->elf.sgotplt->size > 0) {
            bfd_vma sdyn_vma = (sdyn && sdyn->output_section) ?
...
static bool elf_s390_grok_psinfo(bfd *abfd, Elf_Internal_Note *note)
{
    ...
    elf_core_struct *core = elf_tdata(abfd)->core;

    core->pid = bfd_get_32(abfd, note->descdata + 12);
    core->program = _bfd_elfcore_strndup(abfd, note->descdata + 28, 16);
    core->command = _bfd_elfcore_strndup(abfd, note->descdata + 44, 80);

    if (!core->program || !core->command)
        return false;

    char *command = core->command;
...
{
  if (abfd == NULL || buf == NULL || bufsiz == NULL)
    return NULL;
  ...
  switch (note_type)
  {
    case NT_PRPSINFO:
    {
      ...
      if (fname)
      {
        size_t len = strnlen(fname, 16);
        memcpy(data + 28, fname, len);
      }
      if (psargs)
      {
        size_t len = strnlen(psargs, 80);
        memcpy(data + 44, psargs, len);
      }
...
static bfd_vma elf_s390_plt_sym_val(bfd_vma i, const asection *plt, const arelent *rel)
{
  if (plt == NULL)
    return 0;
...
static bool elf32_s390_merge_private_bfd_data(bfd *ibfd, struct bfd_link_info *info)
{
    ...
    if (!elf_elfheader(obfd) || !elf_elfheader(ibfd))
        return false;
